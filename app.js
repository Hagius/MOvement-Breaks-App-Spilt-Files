...
(function() {
  ...
  function startExerciseSegment() {
    ...
    // Create a reference to the current exercise for closure safety
    const exerciseCurrent = current;
    ...
    state.exerciseInterval = setInterval(() => {
      ...
      if (state.exerciseRemaining <= 0) {
        ...
        state.completedExercises.push({
          name: exerciseCurrent.name,
          side: exerciseCurrent.side,
          description: exerciseCurrent.description,
          tag: "nothing"
        });
